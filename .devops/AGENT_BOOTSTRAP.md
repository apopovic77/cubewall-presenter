...
1. **Repository context**
   - Project lives at `/var/code/cubewall-presenter`.
...
4. **Deployment target**
   - Built files go to `/var/www/cubewall/site`.
   - Backups are stored with prefix `cubewall` under `/var/backups/` (configurable via templates).

5. **Startup checklist for new session**
   - `cd /var/code/cubewall-presenter`.
